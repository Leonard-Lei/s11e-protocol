...
contract BodyDetail {
    /// @dev Body N°1 => Human
    function bodyDetailItem1() public pure returns (string memory) {
        return bodyBase("FFEBB4", "FFBE94");
    }

    /// @dev Body N°2 => Shadow
    function bodyDetailItem2() public pure returns (string memory) {
        return bodyBase("2d2d2d", "000000");
    }

    /// @dev Body N°3 => Light
    function bodyDetailItem3() public pure returns (string memory) {
        return bodyBase("ffffff", "696969");
    }

    /// @notice Return the skin name of the given id
    /// @param id The skin Id
    function getBodyItemNameById(uint8 id) public pure returns (string memory name) {
        name = "";
        if (id == 1) {
            name = "Human";
        } else if (id == 2) {
    ...
    /// @dev The bodyBase SVG for the body
    function bodyBase(string memory skin, string memory shadow) private pure returns (string memory) {
